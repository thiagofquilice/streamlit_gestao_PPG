...
def render_evaluation_section(
    *,
    ...
    if latest:
        st.success(f"Nota final da avaliação: {float(latest.get('final_score') or 0):.2f}")
        st.caption(
    ...
    labels = [str(level.get("label", "")) for level in levels]
    value_by_label = {str(level.get("label", "")): float(level.get("value", 0)) for level in levels}

    initial_scores = latest.get("scores", {}) if latest else {}

    with st.form(_state_key("form_eval", target_type, target_id)):
        st.caption(
            "Preencha os critérios abaixo conforme o modelo cadastrado. "
            "Exemplo: selecione níveis mais altos para critérios mais aderentes/qualificados."
        )
        ...
        for idx, criterion in enumerate(form.get("criteria", [])):
            ...
            default_label = next(
                (label for label in labels if value_by_label.get(label) == default_value),
                labels[0],
            )
            ...
            scores[criterion_id] = value_by_label[selected_label]
